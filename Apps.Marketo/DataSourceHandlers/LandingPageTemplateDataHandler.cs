...
public class LandingPageTemplateDataHandler : MarketoInvocable, IAsyncDataSourceHandler
{
    ...
    {
        var request = new RestRequest($"/rest/asset/v1/landingPageTemplates.json", Method.Get);
        var response = await Client.Paginate<LandingPageTemplateDto>(request);
